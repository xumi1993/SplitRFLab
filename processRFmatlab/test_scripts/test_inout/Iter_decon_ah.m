...
odir = fullfile([Station 'ref']);
filename1 = fullfile(odir, [Station 'iternumber.dat']);
fid1=fopen(filename1,'w+');
for i = 1:EV_num
prefix=['ah16cutsac\' char(event(i,:))];disp(['Event number ' num2str(i) ':' prefix])  % prefix of for 3 components to open
% get file names
Zfile = [ prefix, '.1.sac' ];
Efile = [ prefix, '.3.sac' ];
Nfile = [ prefix, '.2.sac' ];
...
[RadialRF, RMS,it_num] = makeRFitdecon_la( Rseis(1:5200), Zseis(1:5200), dt, 5200, Shift, f0, ...
				 niter, minderr);
RFlength=length(RadialRF);

% get time for RF
time = - Shift  + dt*(0:1:RFlength-1);
