...
for i = 1:EV_num
prefix=['ah02cutsac\' char(event(i,:))];disp(['Event number ' num2str(i) ':' prefix])  % prefix of for 3 components to open
% get file names
Zfile = [ prefix, '.1.sac' ];
Efile = [ prefix, '.3.sac' ];
Nfile = [ prefix, '.2.sac' ];
...
%rotate to P-SV-SH corrdinates
[seis, key]=rotateSeisTRZtoPSVSH( seis, rayp(i),5.4, 3);
SVseis = seis(:,2);
Pseis = seis(:,1);

%bandpass
NORD=3;FHP=0.1;FLP=3;
SVseis=bandpassSeis(SVseis, dt, FHP, FLP ,NORD);
Pseis=bandpassSeis(Pseis, dt, FHP, FLP ,NORD);
...
[RadialRF, RMS,it_num] = makeRFitdecon_la( SVseis, Pseis, dt, nt, Shift, f0, ...
				 niter, minderr);
...
odir = fullfile([Station 'SVref']);RFlength=length(RadialRF);
