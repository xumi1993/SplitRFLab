...
if isempty(rf(idx).RadialRF_f1)
    titlestr = 'Broken SAC file or low SNR !!!';
    
rffig = findobj('Tag','RFFigure');
figname = sprintf(['Event: %s; Station: %s; (' num2str(idx) '/' num2str(length(rf)) ')'],eq(idx).dstr, config.stnname);
if isempty(rffig)
...
titlestr = {sprintf(['M_w = %3.1f  Backazimuth: %6.2f' char(186) '  Distance: %6.2f' char(186) '   Depth: %3.0fkm'],rf(idx).Mw, rf(idx).bazi, rf(idx).dis, rf(idx).depth);
            sprintf('SNR_R = %6.2f SNR_T = %6.2f SNR_Z = %6.2f',rf(idx).snrR,rf(idx).snrT,rf(idx).snrZ)};
