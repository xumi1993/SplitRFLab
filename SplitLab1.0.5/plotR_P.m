function plotR_P(Datapath,averageDatapath,OUT_path,h1,h2,k1,k2,sh1,sh2,sk1,sk2,Constantvp,sampling)
...
%% read data
% read average data
%averagefile=strcat(averageDatapath,'\',config.stnname,'average_R.dat');
% dataverage=load(char(averagefile));
...
%% H-k stacking
[stack, stackvar, stack_all, stackall_var ] = hkstack_iwb( datar, -shift, sampling, s.rayp, Moho, kappa, Constantvp,weight );
...
while 1
   ...
   for i=1:RFlength
    ...
    pos=pos*enf;
    fillpos=pos+s.rayp(m);fillneg=neg+s.rayp(m);
    yy = s.rayp(m)*ones(1,RFlength)+edgeColor*enf; 
    hold on
...
while 1
    ...
    receiver=receiver*enf+s.rayp(m);
   %plot lines:
...
set(h2,'xtick',[0:2:80],'xlim',[0 26],'ylim',p2ylim,'ytick',[],'xticklabel',[],'yTickLabel',[]) 
...
%     text(TPsPs(length(TPs))-1,0.084,'PpSs+PsPs');
    ...
    set(h1,'xtick',[0:2:80],'xlim',[0 26],'ylim',[0.038 0.088],'xticklabel',[0:2:80],'FontSize',14,'ytick',[0.04:0.01:0.08],'tickdir','out');
