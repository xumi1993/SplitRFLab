...
h.list = uicontrol(h.dlg, 'style','listbox', 'Position',[40 150 ext(3)+17 figpos(4)-180],...
    'tag','TableList', 'string',[], 'fontname','fixedWidth', 'fontunits','pixel', 'fontsize',10,...
...
'string','Preview','Callback','if config.isoldver;SL_SeismoViewer4old(get(gcbf,''Userdata''));else;SL_SeismoViewer(get(gcbf,''Userdata''));end;');
...
h.button6 = uicontrol( 'style','pushbutton', 'Position',[230 8 60 25],...
    ...
    'Callback','if config.isoldver;SL_showeqstats4old;else;SL_showeqstats;end;');    
