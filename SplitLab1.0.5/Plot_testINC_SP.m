...
[test_SP, ~,~] = makeRFitdecon_la(Rot_seis(i,:,2), Rot_seis(i,:,3), thiseq.dt, RFlength, Shift, f0, ...
				 200, minderr);
%subplot(4,5,i);
%plot(timeaxis,Rot_seis(i,:,2),'k');hold on;set(gca,'xlim',[-10 10],'ylim',[min(seis(:,3)) max(seis(:,3))],'YGrid','on','XGrid','on');
...
xlabel(h10,['Incident angle (' '\circ' ')'],'FontSize',16); ylabel(h10,'Time (s)','FontSize',16);
