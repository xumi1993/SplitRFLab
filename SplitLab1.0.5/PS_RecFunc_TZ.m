...
plot(xlim,[0 0],'g--');
set(gca,'xlim',[-5 80],'xtick',(0:5:80),'Xgrid','on')
...
%
OUT_path = ['/Volumes/LaCie/YN.RFunction/RFresult_0.6/' config.stnname];%path of PRFs
OUT_path1 = ['/Volumes/LaCie/YN.RFunction/RFcutoutdata/' config.stnname];%path of cut out data
% OUT_path = ['/Volumes/GSP1RMCULXF/FJRF/RFresult/' config.stnname];%path of PRFs
% OUT_path1 = ['/Volumes/GSP1RMCULXF/FJRF/RFcutoutdata/' config.stnname];%path of cut out data
...
elseif strcmp(config.FileNameConvention,'YNoldFormat')
    yy=14;ss=30;
elseif strcmp(config.FileNameConvention,'YNFormat')||strcmp(config.FileNameConvention,'CNSFormat')
    yy=1;ss=17;
...
%     datZ = seis1(:,3);
     ...
     if( ~exist( OUT_path1 , 'dir') )
         mkdir( OUT_path1 ); end
     fiddataT = fopen(fullfile(OUT_path1,[thiseq.seisfiles{1}(yy:ss) '_RFdata_T.dat']),'w+'); 
     fprintf(fiddataT,'%20.19f\n',T);
     fclose(fiddataT);
     
     fiddataR = fopen(fullfile(OUT_path1,[thiseq.seisfiles{1}(yy:ss) '_RFdata_R.dat']),'w+'); 
     fprintf(fiddataR,'%20.19f\n',R);
     fclose(fiddataR);
     
     fiddataZ = fopen(fullfile(OUT_path1,[thiseq.seisfiles{1}(yy:ss) '_RFdata_Z.dat']),'w+'); 
