...
extbegin  = floor( (thiseq.phase.ttimes(1) - config.extime_before - o) / thiseq.dt); %index of first element of amplitude verctor of the selected time window
extfinish = floor( (thiseq.phase.ttimes(1) + config.extime_after - o) / thiseq.dt); %index of last element
...
if strcmp(button, 'Yes')
     ...
     fid_finallist = fopen(fullfile(OUT_path,[config.stnname 'finallist.dat']),'a+');
      %OUTPUT Radial RFs
        ...
        fprintf(fid_finallist,'%s %s %f %f %f %f %f %f %f %f\n',thiseq.seisfiles{1}(yy:ss),thiseq.SplitPhase,thiseq.lat,thiseq.long,thiseq.depth,thiseq.dis,thiseq.bazi,Ev_para,thiseq.Mw,config.f0);
     %idx = thiseq.index;
