function out = readseis3D_forRF(config,idx)
...
%read in
efile = fullfile(config.datadir, eq(idx).seisfiles{1});
nfile = fullfile(config.datadir, eq(idx).seisfiles{2});
zfile = fullfile(config.datadir, eq(idx).seisfiles{3});
...
A = mean(A(:) + eq(idx).offset(:));
F = mean(F(:) + eq(idx).offset(:));
...
offset = floor(eq(idx).offset*100)/100;
e(:,1) = e(:,1) + offset(1);
n(:,1) = n(:,1) + offset(2);
v(:,1) = v(:,1) + offset(3);
